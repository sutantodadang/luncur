...
func TestLuncurObjects(t *testing.T) {
	...
	for _, want := range []string{`"ClusterRole"`, "pods/log", "helmchartconfigs", "clusterissuers", "pods/exec", "statefulsets", "metrics.k8s.io", "cronjobs", "persistentvolumeclaims", "horizontalpodautoscalers", "networkpolicies"} {
		if !strings.Contains(all, want) {
			t.Fatalf("manifests missing %q", want)
		}
	}
