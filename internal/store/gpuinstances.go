...
type GPUInstance struct {
	ID         int64
	Provider   string // "vastai"
	ExternalID int64  // provider's contract/instance id
	Label      string
	GPUName    string
	NumGPUs    int
...
func (s *Store) CreateGPUInstance(provider string, externalID int64, label, gpuName string, numGPUs int) (GPUInstance, error) {
	res, err := s.db.Exec(
		`INSERT INTO gpu_instances (provider, external_id, label, gpu_name, num_gpus, status) VALUES (?, ?, ?, ?, ?, 'active')`,
		provider, externalID, label, gpuName, numGPUs,
	)
	if err != nil {
...
func (s *Store) GetGPUInstance(id int64) (GPUInstance, error) {
	row := s.db.QueryRow(
		`SELECT id, provider, external_id, label, gpu_name, num_gpus, status, created_at FROM gpu_instances WHERE id = ?`, id)
...
func (s *Store) ListGPUInstances() ([]GPUInstance, error) {
	rows, err := s.db.Query(
		`SELECT id, provider, external_id, label, gpu_name, num_gpus, status, created_at FROM gpu_instances WHERE status != 'destroyed' ORDER BY id DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func scanGPUInstance(r rowScanner) (GPUInstance, error) {
	var g GPUInstance
	err := r.Scan(&g.ID, &g.Provider, &g.ExternalID, &g.Label, &g.GPUName, &g.NumGPUs, &g.Status, &g.CreatedAt)
